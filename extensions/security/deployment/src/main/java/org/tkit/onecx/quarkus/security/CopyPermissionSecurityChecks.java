...
interface CopyPermissionSecurityChecks {
    ...
    final class PermissionSecurityChecksBuilder {
        ...
        CopyPermissionSecurityChecks build() {
                    ...
                    for (PermissionKey key : keyList) {
                        if (!classSignatureToConstructor.containsKey(key.classSignature())) {
                            ...
                            if (constructor.parametersCount() == 0
                                    || !DotNames.STRING.equals(constructor.parameterType(0).name())) {
                                throw new RuntimeException(
        ...
        private static void gatherPermissionKeys(AnnotationInstance instance, MethodInfo methodInfo,
            ...
            for (String permissionToAction : instance.value().asStringArray()) {
                if (permissionToAction.contains(PermissionsAllowed.PERMISSION_TO_ACTION_SEPARATOR)) {

                    // expected format: permission:action
                    final String[] permissionToActionArr = permissionToAction
                            .split(PermissionsAllowed.PERMISSION_TO_ACTION_SEPARATOR);
                    if (permissionToActionArr.length != 2) {
                        throw new RuntimeException(String.format(
                                ...
                                permissionToAction, PermissionsAllowed.PERMISSION_TO_ACTION_SEPARATOR));
                    }
        ...
        private static final class PermissionKey {
            ...
            private boolean notAutodetectParams() {
                return !(params.length == 1 && PermissionsAllowed.AUTODETECTED.equals(params[0]));
            }
        ...
        private static final class PermissionCacheKey {
            ...
            private static boolean secondParamIsNotStringArr(MethodInfo constructor) {
                return constructor.parametersCount() < 2 || constructor.parameterType(1).kind() != Type.Kind.ARRAY
                        || !constructor.parameterType(1).asArrayType().constituent().name().equals(DotNames.STRING);
            }
